...
RSpec.describe CampaignTargetPolicy do
  ...
  context 'user is site admin' do
    ...
    permissions :show? do
      it 'allows access to any user' do
        expect(subject).to permit(user, campaign_target)
      end
    end

    permissions :create? do
      it 'allows campaign_target create' do
        expect(subject).to permit(user, campaign_target)
      end
    end

    permissions :destroy? do
      it 'allows campaign_target destroy' do
        expect(subject).to permit(user, campaign_target)
      end
    end
  end
